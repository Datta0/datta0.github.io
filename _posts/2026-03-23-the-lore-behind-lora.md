...
In practice, these are called `adapters`. 
...
So at training time, you benefit a lot from split LoRA. But at inference time, if you are sure of only serving the updated model, you are better of merging the weights and creating a final model and serving it. This reduces the compute done in inference path and can speed up inference by up to 30% on vLLM. Source: Trust me bro :)
...
- Enabling or disabling adapter is easy. You just skip the $W_{delta}$ computation and you get the original model back :)
